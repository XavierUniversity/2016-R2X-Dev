...
<cffunction name="init" access="public" returntype="R2XStudentProcess2014" output="false">
	...
	*** --->
	...
	<cffunction name="getListCount" returnType="string">
		<cfargument name="BannerID" required="true">
		<cfargument name="itemlist" required="true">
		
		<cfset itemCount = 0 >
		
		<cfloop index = "StepItem"  list="#itemlist#">
			
		 <cfif not isProcessItemComplete(bannerid,StepItem )> 
	...
	<cffunction name="getBlogCount" returnType="string">
		<cfargument name="BannerID" required="true">
		<cfargument name="itemlist" required="true">
		
		<cfset blogCount = 0 >
		
		<cfloop index = "blogitem"  list="#itemlist#">
			
		 <cfif not isProcessItemComplete(bannerid,blogitem )> 
	...
	<cffunction name="findScheduleSubmit" returnType="string">
			<cfargument name="BannerID" required="true">
			<cfargument name="EntryTerm" required="true" >
		
			<cfset completeSchedule = 0 >
			
			<cfquery name="getAdvisingReady" dataSource="roadtoDB">
				SELECT BannerID,itemcode from ProcessStudent where bannerid='#bannerid#' and itemcode='advisingReady' and date_deleted is null
			</cfquery>
			
			<cfquery name="getAdvising" dataSource="roadtoDB">
				select * FROM advisingSchedules where bannerid= <cfqueryparam CFSQLType="CF_SQL_NUMERIC" value="#Bannerid#" > 
			</cfquery>	
			
			<cfif getAdvising.recordcount is 0  and getAdvising.recordcount is 0 >
				
			<cfquery name="getSFRSTCR" datasource="ERP">
				select sum(sfrstcr_credit_hr) as totalCredits
				from sfrstcr
				where sfrstcr_pidm = (select spriden_pidm from spriden where spriden_id = <cfqueryparam value="#BannerID#" cfsqltype="cf_sql_varchar"> and spriden_change_ind is null)
				and sfrstcr_term_code = <cfqueryparam value="#EntryTerm#" cfsqltype="cf_sql_varchar">
				and sfrstcr_rsts_code in ('RW','RE','WL')
			</cfquery>


				<cfif getSFRSTCR.totalCredits gte 12  >
					
					<cfset completeSchedule = 'FULL' >
					
					<cfelseif getSFRSTCR.totalCredits lt 12 and getSFRSTCR.totalCredits is not "" >
					
					<cfset completeSchedule = 'PART' >
					
					<cfelse>
					
					<cfset completeSchedule = 'NONE' >
...
<!--- *** Process Item Code: parentAccess - Trad, Int'l *** --->
	...
	<!--- This must be done for each step --->
		...
		<cfset stepStruct.timeframe = ''>
		<cfset stepStruct.title = 'Complete Your Math Placement Test'>
		<cfif this.isProcessItemComplete(session.bannerid,stepStruct.itemCode)>
			<cfset stepStruct.status = 'Complete'>
		<cfelse>
			<cfset stepStruct.status = 'Active'>
	...
	<cftry>
		...
		<cfset aleksLink = this.generateAleksLink(session.username)>
		
		<cfparam name="getPlacement.SORTEST_TEST_SCORE" default="UNKNOWN">
		<cfswitch expression="#getPlacement.SORTEST_TEST_SCORE#">
			<cfcase value="105">
				<cfset placementOut = 'MATH 105. <b>This class counts as a general elective, but does not satisfy the core requirement for math. If you have not done so already, please complete the <a href="#aleksLink#" target="_blank">ALEKS Prep and Learning Module</a>, in an attempt to raise your score</b>'>
			</cfcase>
			<cfcase value="PRE">
		...
		<cfif getPlacement.recordCount EQ 1>
			...
			<cfif getPlacement.SORTEST_TEST_SCORE EQ '105'>
				<p><b>Based on your placement level, it is in your best interest to complete a <cfoutput><a href="#aleksLink#" target="_blank">Prep and Learning Module</a></cfoutput> within the ALEKS system which may result in achieving a higher math placement level.</b>  If you choose to do this you will be required to spend at least 8 hours in the ALEKS system working to complete the module.</p>
				...
				<p>Access to the <cfoutput><a href="#aleksLink#" target="_blank">ALEKS system</a></cfoutput> will remain open to you via this screen.</p>
			...
			<cfelse>
				<p>Based on your placement level, you are eligible, but not required, to complete a <cfoutput><a href="#aleksLink#" target="_blank">Prep and Learning Module</a></cfoutput> within the ALEKS system which may result in achieving a higher math placement level.  If you choose to do this you will be required to spend at least 8 hours in the ALEKS system working to complete the module.</p>
				...
				<p>Access to the <cfoutput><a href="#aleksLink#" target="_blank">ALEKS system</a></cfoutput> will remain open to you via the this screen.</p>
			</cfif>
...
Our online tests are only for placement purposes. But that doesn’t mean they aren’t important. Take them seriously – your performance will serve as a guide so you and your advisor can select the courses that best suit your needs for the first semester schedule. The tests must be completed before you can register for classes.
	...
	<cffunction name="getNextStepsPart2" returntype="array">
        ...
        <cfif isdefined("url.showAll") OR (this.isProcessItemComplete(session.bannerid,'deposit') and NOT this.isProcessItemComplete(session.bannerid,'housingRoomSelection') and  datecompare(compDate, Application.housing_room_selection_start) gt 0 )>
			<!--- This must be done for each step --->
            <cfset stepStruct = {}>
                    <cfset stepStruct.itemCode = 'housingRoomSelection'>
                    <cfset stepStruct.topic = 'Housing'>
                    <cfset stepStruct.status = 'manual'>
                    <cfset arrayAppend(nextStepsArray,stepStruct)>
		</cfif> 
		...
		<!--- *** Process Item Code: xavierEmail - Preview - ALL *** --->
		<cfif isdefined("url.showAll") OR (NOT this.isProcessItemComplete(session.bannerid,'deposit') OR datecompare(compDate, Application.xavier_email_start) lt 0)>
			<!--- This must be done for each step --->
			<cfset stepStruct = {}>
			<cfset stepStruct.itemCode = 'xavierEmail'>
			<cfset stepStruct.status = 'Preview'>
			<cfset stepStruct.type = ''>
			<cfset stepStruct.typeText = ''>
			...
			<cfset stepStruct.title = "Access Your Xavier Email">
			<cfsavecontent variable="stepStruct.body">
				<p>Now that you're in, it's time to think about how we're going to communicate with you.</p> 

				<p>Spoiler alert: It's by email. You need to access your Xavier email and start checking it at least once a week. Many of our systems send messages to your Xavier email, including Self-Service, Housing Self-Service, and campus-wide announcements. </p>
			</cfsavecontent>
			<cfset stepStruct.topic = 'ToDo'>
			<cfset arrayAppend(nextStepsArray,stepStruct)>
		</cfif>
        ...
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.xavier_email_start) ge 0))>
			<!--- This must be done for each step --->
            <cfset stepStruct = {}>
			<cfset stepStruct.itemCode = 'xavierEmail'>
			...
			<cfsavecontent variable="stepStruct.body">
				...
				<p>So how do you access your Xavier email? Click the email link on the student hub at <a href="http://www.xavier.edu/students/" target="_blank">http://www.xavier.edu/students/</a>.</p>
                    
				<cfif session.parentlogin is FALSE >
					<p><a href="http://www.xavier.edu/students/" target="_blank" class="button radius">Access Your Xavier Email</a></p>
				</cfif>	
                    
            </cfsavecontent>
			<cfset stepStruct.link = "http://www.xavier.edu/students/" >
			<cfset stepStruct.topic = 'ToDo'>
			<cfset arrayAppend(nextStepsArray,stepStruct)>
        </cfif>
		<!--- /end Xavier Email --->
...
<!--- 		<!--- *** Process Item Code: housingPrefManual - Preview - N,I *** --->
	...
	<!--- *** Process Item Code: acPhotoUploaded - Preview - ALL *** --->
		...
		<cfif isdefined("url.showAll") OR (this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.ac_photo_start) ge 0)>
			...
			<cfset stepStruct.timeframe = '2016-07-31'>
			<cfset stepStruct.title = 'Upload Your ALL Card Photo'>
			<cfsavecontent variable="stepStruct.body">
	...
	<p><a href="http://www.xavier.edu/auxiliary-services/all-card-center.cfm" target="_blank">&raquo; Learn More About the ALL Card</a></p>
		...
		<cfif isdefined("url.showAll") OR (NOT this.isProcessItemComplete(session.bannerid,'deposit') OR datecompare(compDate, Application.namecoach_start) lt 0)>
			<!--- This must be done for each step --->
			<cfset stepStruct = {}>
        ...
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.namecoach_start) ge 0))>
			...
			<cfsavecontent variable="stepStruct.body">
				<p>Basically, it means everything - and we want to make sure we get yours right. So tell us how to pronounce it - so we know we're doing it correctly. Record your name using our pronunciation service, NameCoach, and it will be used throughout your time at Xavier.</p>

				<p>Example: Xavier. It's pronounced Zey-vyer. Get it?</p>
				
				<p>Please record your full name, not just your first name.</p>
                    
				<cfif session.parentlogin is FALSE >
					<p><a href="<cfoutput>#namecoachLink#</cfoutput>" target="_blank" class="button radius">Record Your Name</a></p>
				</cfif>	
        ...
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit,advisingSRStatement') and datecompare(compDate, Application.plagiarism_start) ge 0) and session.studentType NEQ 'T' and NOT session.international)>
			...
			<cfsavecontent variable="stepStruct.body">
				...
				<cfif session.parentlogin is FALSE >
					<p><a href="https://roadto.xavier.edu/plagiarism/index.cfm" class="button radius">Take the Preventing Plagiarism Tutorial</a></p>
				</cfif>	
    ...
    <!--- smooth transitions --->
        ...
        <cfif (isdefined("url.showAll") OR (datecompare(compDate, Application.st_start) ge 0 and (session.ethnicityID EQ '1' or session.ethnicityID EQ '2' or session.ethnicityID EQ '3' or session.ethnicityID EQ '4' or session.ethnicityID EQ '8') and this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.st_stop) lt 0 and session.studentType NEQ 'T' and not session.international))>
            ...
            <cfset stepStruct.typeText = 'Register for Smooth Transitions to Complete'>
                    <cfset stepStruct.timeframe = '2016-06-27'>
        ...
        <cfif isdefined("url.showAll")  OR (this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.student_commitment_start) ge 0) >
            ...
            <cfset stepStruct.typeText = 'Sign the Student Commitment'>
                    <cfset stepStruct.timeframe = '2016-08-12'>
		...
		<!--- *** Process Item Code: TransferOrientation - Tran *** --->
	...
	<!--- *** Process Item Code: wellnessMedicalHistory - Preview - ALL *** --->
		<cfif isdefined("url.showAll") OR  (not this.isProcessItemComplete(session.bannerid,'deposit') or datecompare(compDate, Application.wellness_general_start) lt 0) >
			<!--- This must be done for each step --->
			<cfset stepStruct = {}>
			...
			<cfset stepStruct.type = ''>
			<cfset stepStruct.typeText = ''>
			<cfset stepStruct.timeframe = '<span class="timeframe-label">Approximate Start:</span> #this.generalMonth(application.wellness_general_start)#'>
    ...
    <!--- Auxiliary Services - Bookstore --->
        ...
        <cfif isdefined("url.showAll") OR (this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.bookstore_start) ge 0)>
            ...
            <cfset stepStruct.typeText = ''>
                ...
                <p><a href="https://www.youtube.com/watch?v=Sf76wf1iVNc" target="_blank"> &raquo; Watch Archived Bookstore Muskie Chat</a> </p>
    ...
    <!--- Wellness - Health Insurance Waiver --->
        ...
        <cfif isdefined("url.showAll") OR (this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.wellness_waiver_start) ge 0)>
            ...
            <cfset stepStruct.typeText = ''>
                ...
                <ol class="inItem">
                    ...
                    <li>Click on Enroll/Waive and follow the instructions. Your Student ID Number is : <strong><cfoutput>#session.bannerid#</cfoutput></strong></li>
		...
		<!--- *** Process Item Code: communicationsSurvey - ALL *** --->
            ...
            <cfelse>
            	<cfset stepStruct.status = 'Active'>
                    ...
                    <cfset stepStruct.type = 'manual'>
            <cfset stepStruct.typeText = ''>
                    <cfset stepStruct.timeframe = '2017-06-15'>
        ...
        <cfif isdefined("url.showAll") OR (this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.parking_permit_start) ge 0)>
            ...
            <cfset stepStruct.typeText = ''>
                    ...
                    <cfsavecontent variable="stepStruct.body">
                <p> The Office of Auxiliary Services is now offering the 2016-2017 Xavier Parking Permits online. </p>
        ...
        <!--- end Retention Countdown section --->                
        ...
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.thinkAboutIt_start) ge 0))>
            ...
            <cfset stepStruct.typeText = ''>
                    ...
                    <p>Xavier is committed to providing a safe, nurturing environment where students can grow and learn. 
	                   As part of this commitment, all incoming students will complete Think About It, a program aimed 
	                   at preparing you for the unique challenges and responsibilities of college life. That focuses on 
	                   minimizing risks associated with alcohol, drugs, and sexual violence. </p>
	                   
 
					   <p>Think About It is an online course that will take approximately 2.5 hours to complete. <br><strong>You can complete 
						   the course on your own time and can save and exit at any time. You must complete this course prior to your arrival
						    at Xavier.</strong> In the course, there are a series of questions to assist Xavier in our programming.</p>
						    
						    <p> The information
						     collected is strictly confidential—we will not receive any personally identifiable information about you.</p>
                   
                   
                    <p style="font-weight: bold">Next steps:</p>
                <ol>
				  <li><a href="http://campusclarity.com/" target="_blank">Visit the Think About It web site.</a></li>
				  <li>Click the blue box in the upper right corner of the screen that says “Login to Training.”</li>
				  <li>Use your Xavier University email to access your account.<br>Your Xavier email address is: <strong><cfoutput>#session.username#</cfoutput>@xavier.edu</strong></li>
				  <li><a href="http://mail.xavier.edu">Login to your Xavier Email</a> and authenticate your account.</li>
				  <li>Once you authenticate your account, you will be able to create a password and <a href="http://campusclarity.com/" target="_blank">login to your Think About It account.</a></li>
           		</ol>
              <!---            <ol>
            ...
            <p>You will receive regular reminders from Campus Clarity to your Xavier email account as you progress through the course, so check your Xavier email reguarly.</p>
            <p>We hope that you enjoy the course!</p>
                   ...
                   <cfif session.parentlogin is FALSE >
                    	<p><a href="http://campusclarity.com/" class="button radius">Create your "Think About It" account now.</a></p>
                   </cfif>
        ...
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.ferpa_proxy_access) ge 0))>
			<!--- This must be done for each step --->
            <cfset stepStruct = {}>
            ...
            <cfelse>
            	<cfset stepStruct.status = 'Active'>
                    ...
                    <cfset stepStruct.type = 'manual'>
            <cfset stepStruct.typeText = 'Add You FERPA Proxy'>
                    <cfset stepStruct.timeframe = ''>
                    <cfset stepStruct.title = 'FERPA Proxy Access'>
                    <cfsavecontent variable="stepStruct.body">
             <p>Xavier can't share certain information of yours with anyone else unless you give permission under the <a href='http://www.xavier.edu/registrar/ferpa.cfm'>Family Educational Rights and Privacy Act</a> (FERPA).
While not required, you may grant permission to a parent, guardian or other individual, so that person can see your financial aid status or pay your tuition bill. This is called Proxy Access.</p>

					<cfif session.parentlogin is FALSE >
                    	<p><a href="https://roadto.xavier.edu/your-road/parent-proxy.cfm" class="button radius">Make your proxy selections</a></p>
					</cfif>
						
            </cfsavecontent>
                    <cfset stepStruct.link = "https://roadto.xavier.edu/your-road/parent-proxy.cfm">
                    <cfset stepStruct.topic = 'ToDo'>
                          <cfset arrayAppend(nextStepsArray,stepStruct)>
        </cfif>

        <!--- *** Process Item Code: bursarFERPA - ALL *** --->
        <cfparam name="Application.bursar_ferpa_release" default="2016-07-13 00:00:00.0" >
        
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.bursar_ferpa_release) ge 0))>
			<!--- This must be done for each step --->
            <cfset stepStruct = {}>
            ...
            <cfset stepStruct.typeText = ''>
            	...
            	<p>The Family Educational Rights and Privacy Act (FERPA), passed in 1974, is a federal law that protects the privacy of student education records. This law gives students 18 years of age or older, or students of any age if enrolled in any postsecondary educational institution, the right to privacy of their education records. For a comprehensive discussion of FERPA at Xavier please visit <a href="http://www.xavier.edu/registrar/ferpa.cfm">http://www.xavier.edu/registrar/ferpa.cfm</a>.</p>
                            ...
                            <p>Students are encouraged to complete Part A of the FERPA form and return it to the Office of the Bursar. If the student does complete Part A, then Part B does not have to be completed. However, if the student decides not to sign the FERPA form, the parent or guardian can complete <a href="http://www.xavier.edu/bursar/documents/FERPA-201501.pdf"  target="_blank">Part B of this form</a> and attach a signed copy of their 2015 Federal Income Tax Return to certify that the student is their dependent according to Section 152 of the Internal Revenue Code. Part B is valid for only the current academic year. Since IRS dependency can change annually, you must submit copies of your Federal Income Tax Return each year along with this form to continue access to your student&rsquo;s financial and education records.</p>
                
							<cfif session.parentlogin is FALSE >
								<p><a href="https://roadto.xavier.edu/your-road/bursar-ferpa.cfm" class="button radius">Complete the Bursar FERPA Release Now</a></p>
							</cfif>
                
                <p>OR</p>
                
                <p><a href="http://www.xavier.edu/bursar/documents/FERPA-201501.pdf" target="_blank">Download the FERPA form</a></p>
                            
                <p><strong>Return the completed form by scanning it to:</strong> <a href="mailto:xubursar@xavier.edu">xubursar@xavier.edu</a></p>
                <p><strong>OR faxing the completed form to:</strong> 1-513-745-2926</p>
                <p><strong>OR mail the completed form to:</strong><br />
                The Office of the Bursar<br />
    ...
    <!--- Safety --->
        ...
        <cfif isdefined("url.showAll") OR ((this.isProcessItemComplete(session.bannerid,'deposit') and datecompare(compDate, Application.safety_start) ge 0))>
            ...
            <cfset stepStruct.typeText = ''>
                    ...
                    <cfsavecontent variable="stepStruct.body">
                     <p>Families and students are always concerned about safety and want to know &ldquo;How safe is the Xavier campus?&rdquo; simply and directly, <strong>Very Safe</strong>. 

			<p>The University provides a voice, text message, and email system, <strong>XU ALERT ME</strong>. This communication system contacts members of the campus community through voice, text messages and emails in the event of an urgent situation. Students may sign up for the service through the MyXU campus portal.</p>
...
<!--- international orientation --->
    ...
    <!--- Manresa --->             		<!--- *** Process Item Code: manresa - Preview - Trad, Intl *** --->
		...
		<!--- moved to an inbox message in 2016
		<!--- Student Employment --->
...
<cffunction name="generateAleksLink" returntype="string">
	<cfargument name="usernameIn">
	
    <cfset crypto = new library.crypto() />
	<cfset todayDate="#DateFormat(now(),'YYYY-MM-DD')#" >
	<cfset ssoAccount="xavier" >
	<cfset passPhrase="41dc5ab20f1043a85963145d8834877f" >
	<cfset ssoCode = "#todayDate#:#ssoAccount#:#arguments.usernameIn#" />
	<cfset sso_sig= "#crypto.hmacSha1( passPhrase, ssoCode, "hex" )#" >
	<cfset sso_key="#todayDate#:#ssoAccount#:#arguments.usernameIn#:#sso_sig#">
	
	<cfreturn "https://secure.aleks.com/ssologon?sso_key=#sso_key#&register_firstname=#session.firstName#&register_lastname=#session.lastName#&register_email=#session.email#&enroll_class_code=#application.advALEKScohort#">
